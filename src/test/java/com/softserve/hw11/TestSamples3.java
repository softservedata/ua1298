...
public class TestSamples3 {
    ...
    @ParameterizedTest
    // UPDATED (@CsvFileSource instead of @CsvSource)
    @CsvFileSource(resources = "/valid_login_data.csv", numLinesToSkip = 1)
    ...
    @ParameterizedTest
    // UPDATED (@CsvFileSource instead of @CsvSource)
    // Added more invalid credentials in .csv file.
    // Added try/catch in the code block.
    @CsvFileSource(resources = "/invalid_login_data.csv", numLinesToSkip = 1)
    ...
    public void testInvalidSignIn(String email, String password, String expectedMessage) {
        try {
            ...
            Assertions.fail("Test iteration failed for email: " + email + ", password: " + password + " with exception: " + e.getMessage());
        }
    }
}